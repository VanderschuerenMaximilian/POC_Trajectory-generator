<script lang="ts">
  ...
  import { type LayoutOptions, type ElkNode, type ElkExtendedEdge } from 'elkjs';
    ...
    {#if nodes}
        {#each nodes as node, i}
            {#if node.type === NodeTypes.trajectory}
                ...
                <EventNode {node} />
                {:else if node.type === NodeTypes.step}
                <StepNode {node} />
                {:else if node.type === NodeTypes.option}
                <OptionNode {node} />
                {:else if node.type === NodeTypes.datapoint}
                <DatapointNode {node} />
            {/if}
        {/each}
    {/if}
